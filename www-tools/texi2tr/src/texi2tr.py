...
def doIgnore (content, state):
    if state == ignore:
        return doConsume(content, state, 'ignore')
    else:
...
def doIfnothtml (content, state):
    if state == ignore:
        return doConsume(content, state, 'ifnothtml')
    else:
        pushState('ifnothtml', state)
        return '', ignore
...
def doGroup (content, state):
    if state == ignore:
        return doConsume(content, state, 'group')
    else:
        pushState('group', state)
        return '', state

def doPass (content, state):
    return content, state

def doImage (content, state):
    return "", state
...
def doDirentry (content, state):
    if state == ignore:
        return doConsume(content, state, 'direntry')
    else:
...
def doDfn (content, state):
    if state == ignore:
        return '', ignore
    else:
        return content, state

def doSc (content, state):
    if state == ignore:
        return '', ignore
    else:
        return content, state

def doAcronym (content, state):
    if state == ignore:
        return '', ignore
    else:
        return content, state
...
def populateFunctions ():
    ...
    functions['insertcopying'] = doPass
    functions['image'] = doImage
    ...
    functions['ifnothtml'] = doIfnothtml
    ...
    functions['syncodeindex'] = doPass
    functions['table'] = doTable
    endFunctions['table'] = doTableEnd
    functions['TeX'] = doTeX
    functions['top'] = doTop
    functions['title'] = doPass
    functions['subtitle'] = doPass
    functions['author'] = doPass
