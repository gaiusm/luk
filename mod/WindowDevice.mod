...
FROM ColourText IMPORT WriteCharacterAndAttribute ;
FROM Colours IMPORT LightGrey, Black ;
...
CONST
   MaxWidth   = 79 ;
   MaxHeight  = 25 ;
   BoarderCol = LightGrey*010H+Black ;  (* LightGrey background; Black foreground *)
...
TYPE
   ...
   window = RECORD
               ...
               BgCol   : CARDINAL ;
               FgCol   : CARDINAL ;
               Attrib  : CARDINAL ;   (* Attribute made from Bg and Fg   *)
...
BEGIN
   ...
   WITH w^ DO
      ...
      Width := width ;
      Height := height ;
      BgCol := bg ;   (* Maybe - place into an attribute *)
      FgCol := fg ;
      Attrib := bg*010H+fg ;  (* ok have done *)
      Border := border ;
      Xcursor := 0 ;
      Ycursor := 0 ;
      StrCopy( '', Title ) ;
...
BEGIN
   ...
   IF Top=NIL
   THEN
      MonStrIO.WriteString('Window error: SubWindow there are no more windows left\n') ;
   ELSE
      (* Quite Subtle - t guarenteed to point to a window *)
(*    WHILE t#Top DO
         RefreshWindow( t ) ;
         t := t^.Down
      END
...
PROCEDURE WriteCharAbs (ch: CHAR; col: CARDINAL; x, y: CARDINAL) ;
BEGIN
(*
...
*)
   WriteCharacterAndAttribute( x, y, ch, col )
END WriteCharAbs ;
