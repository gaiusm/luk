...
FROM SYSTEM IMPORT ADDRESS, PROCESS, LISTEN, OnOrOff, TurnInterrupts,
...
BEGIN
   IF NOT c
   THEN
      ; DebugString('IdleProcess = ') ; CardToStr(CARDINAL(IdleProcess), 0, db) ;
      DebugString(db) ; DebugString('\n') ;

      DebugString('Vols  = ') ; CardToStr(CARDINAL(IdleProcess^.Volatiles), 0, db) ;
      DebugString(db) ; DebugString('\n') ;
      Ps ;
      Halt('Assert failed', line, file)
   END
END Assert ;
...
BEGIN
   ...
   WITH d^ DO
      ...
      ELSE
         ...
         DebugString('trying to resume a process which is not suspended') ;
...
BEGIN
   WITH p^ DO
      ...
      END ;
      DebugString('  Stack usage (') ;
      IF Size=0
      THEN
         DebugString('  0%)\n')
      ELSE
         CardToStr((CARDINAL(Volatiles) - CARDINAL(Start)) * 100 DIV Size, 3, a) ;
         DebugString(a) ;
         DebugString('%)\n')
      END
   END
...
BEGIN
   ...
   FOR Pri := idle TO hi DO
      ...
      END
   END ;
   Assert(Highest#NIL, __FILE__, __LINE__) ;
   RETURN( Highest )
END NextReady ;
...
BEGIN
   NEW(CurrentProcess) ;
   WITH CurrentProcess^ DO
      Size := 0 ;              (* init uses the default stack and      *)
